...
> 最后同步：2026-09-18 | 基于 git commit 历史自动标记
>
> **状态：v3-review-complete**（2026-09-18）。v2 的五项优化已完成；之后新增「自定义外观」与「MP4 动态背景」两个功能，并按审查结论完成一批修复（安全、功能可用性、IO 安全、代码卫生）。当前 83 Rust + 175 前端测试全绿。
>
> ✅ = 已完成 | ◐ = 部分完成 | ❌ = 未开始
...
| P5 测试门禁 | Rust/前端测试、冒烟流程 | ✅ 100% | 83 Rust + 175 前端测试、冒烟脚本（9 步）、CI/CD（7 步冒烟）、Husky + Prettier、发布检查清单 | — |
...
## v3：新增功能与审查修复（2026-09）

### 新增功能 ✅

| 功能 | 说明 |
|---|---|
| 自定义外观 | 自选背景色、主题色、背景图片；毛玻璃效果（可调透明度，作用域可选全局 / 仅内容区） |
| MP4 动态背景 | 自动循环播放；暂停 / 停止（截图并释放视频内存）/ 恢复（从暂停时间线继续）/ 切换静态壁纸；图片 5MB、视频 50MB 上限 |

### 审查修复 ✅

| # | 类别 | 修复内容 |
|---|---|---|
| 1 | 功能 | 接线视频 `stop()`/`resume()`——此前控件从未被调用，截图/释放/恢复流程完全不可达 |
| 2 | 功能 | 截图竞态（`readyState` 守卫）与 `toDataURL` 异常兜底 |
| 3 | 功能 | 修正 `video_playing` 语义（停止/清除/重置应为 false） |
| 4 | 功能 | `video_time` 写入防抖（约 1s），并在暂停/停止时 flush |
| 5 | 安全 | Web 端文件读取绑定到已保存的笔记根目录，`canonicalize` + 前缀校验，笔记读取限 Markdown，asset 加 `nosniff` |
| 6 | 正确性 | 毛玻璃开启时保留自定义背景色（`withAlpha` 合成） |
| 7 | 正确性 | 主题字段清空时 CSS 变量回落默认值 |
| 8 | 正确性 | 统一背景图渲染（移除重复注入的 DOM 层），并走 `convertFileSrc` |
| 9 | 数据 | `mistakes.jsonl` 写入加进程级锁，消除并发丢更新 |
| 10 | 性能 | backup / restore / data-status / session-cleanup 移入 `spawn_blocking` |
| 11 | 卫生 | 韩文 locale 错别字、连接失败文案 i18n、移除重复 store 别名、Tauri 检测类型安全化 |

### 已知遗留（未处理）

- 毛玻璃尚未给面板本身加 `backdrop-filter`（当前模糊作用于背景层）。
- 大笔记库首次扫描仍可能卡顿（已有增量索引缓解）。
- 题目生成仍非真正逐题流式（NDJSON 方案 A 已确认维持现状）。
- 桌面模式读取文件未做根目录绑定（信任边界不同，仅 Web 端为网络暴露面）。
- `MarkdownContent.vue` 与 `MarkdownRenderer.vue` 各自持有 MarkdownIt 实例（配置有意不同，未强行统一）。
