...
- ✅ 选项文本答案（如 `Alpha`、`Alpha and Gamma`、`Alpha、Gamma`）与字母答案（如 `A`、`A,C`）在计分和选项高亮中使用同一套归一规则
...
- ✅ QuizGenerator 会归一损坏的题型、语言和难度默认值，避免非法 settings 污染生成请求
- ✅ 答案归一与选项高亮一致性测试（含本地化分隔符、英文 `and` 连接的选项文本答案）
...
| P3 LLM 稳定性 | JSON 校验、流式展示、能力探测 | ✅ ~95% | schema 校验、多题型支持、答案归一/高亮一致性（含选项文本、本地化分隔符与英文 `and` 连接）、raw 日志、阶段进度、自动修复、能力探测 | — |
...
| P5 测试门禁 | Rust/前端测试、冒烟流程 | ✅ 100% | 54 Rust + 142 前端测试、冒烟脚本、CI/CD、README 备份文档、发布检查清单 | — |
