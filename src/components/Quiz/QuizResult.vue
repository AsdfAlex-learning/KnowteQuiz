          ...
          <span
            ...
            :class="
              isCorrect(q)
                ...
                : 'bg-[var(--color-error)] text-[var(--bg-base)]'
            "
          >
            {{ isCorrect(q) ? '\u2713' : '\u2717' }}
          </span>
...
async function handleSaveMistake(q: QuizQuestion) {
  ...
  const questionReasoning =
    props.mode === 'advanced' ? (quizStore.advancedReasoningFor(q.id) ?? props.reasoning) : props.reasoning;
...
function diagnosisFor(questionId: string): DiagnosisContext | undefined {
  ...
  return (
    quizStore.diagnosisContextFor(questionId) ??
