      ...
      >
        ...
        >
          ...
          <div class="flex-1 overflow-y-auto px-6 py-4 space-y-6">
            <!-- Tab Navigation -->
            <div class="flex gap-1 p-1 rounded-lg bg-[var(--bg-elevated)] border border-[var(--border-default)]">
              <button
                v-for="tab in tabs"
                :key="tab.key"
            ...
            <template v-if="activeTab === 'appearance'">
              <AppearanceSettings :model-value="localSettings.theme_config" @update:model-value="updateThemeConfig" />
            </template>
...
import type { ConnectionTestResult, Settings, SettingsLLM, SettingsQuiz, ThemeConfig } from '@/types/settings';
...
const tabs = computed(() => [
  { key: 'general', label: t('settings_page.general_tab') },
  { key: 'appearance', label: t('settings_page.appearance_tab') },
]);
...
function updateThemeConfig(themeConfig: ThemeConfig) {
  localSettings.theme_config = themeConfig;
}
