<template>
  <div>
    <!-- Directory node -->
    <div v-if="node.is_dir" class="select-none">
      <div
        class="tree-item"
      ...
      <div v-show="isExpanded">
        <FileTreeItem v-for="child in node.children" :key="child.path" :node="child" :depth="depth + 1" />
      </div>
    </div>
    ...
    >
      <!-- File icon -->
      <svg
        class="w-4 h-4 flex-shrink-0 text-[var(--text-muted)]"
        viewBox="0 0 16 16"
        fill="none"
        stroke="currentColor"
        stroke-width="1.2"
