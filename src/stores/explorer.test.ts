...
describe('explorer store workspace persistence', () => {
  ...
  it('scans and persists a manually entered root path', async () => {
    ...
    expect(noteService.scanNotes).toHaveBeenCalledWith('/notes');
    expect(store.rootPath).toBe('/notes');
    expect(store.tree).toEqual([{ name: 'rust.md', path: '/notes/rust.md', is_dir: false, children: [] }]);
    expect(settingsService.saveSettings).toHaveBeenCalledWith({
      ...settings,
      workspace: {
        ...settings.workspace,
  ...
  it('ignores stale root scans when a newer root path finishes first', async () => {
    ...
    const newTree = [{ name: 'new.md', path: '/new/new.md', is_dir: false, children: [] }];
    vi.mocked(noteService.scanNotes).mockReturnValueOnce(firstScan.promise).mockReturnValueOnce(secondScan.promise);
    const store = useExplorerStore();
    ...
    firstScan.resolve([{ name: 'old.md', path: '/old/old.md', is_dir: false, children: [] }]);
    await firstOpen;
