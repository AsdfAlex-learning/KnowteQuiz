...
pub async fn scan_directory(root_path: &str) -> Result<Vec<NoteTreeNode>, String> {
    let root = Path::new(root_path);
    let metadata = fs::metadata(root)
        .await
        .map_err(|_| format!("Directory does not exist: {}", root_path))?;
    if !metadata.is_dir() {
...
async fn collect_index_entries(
    ...
    for node in nodes {
        ...
        let title = fs::read_to_string(path).await
...
async fn scan_recursive(dir: &Path, _root: &Path) -> Result<Vec<NoteTreeNode>, String> {
...
mod tests {
    ...
    async fn scan_directory_with_index_keeps_tree_when_note_content_cannot_be_indexed() {
        ...
        let index_text = std_fs::read_to_string(data_dir.join("index.json"))
    ...
    #[tokio::test]
    async fn incremental_scan_reuses_unchanged_file_titles() {
        ...
        std_fs::write(root.join("note.md"), note_text).expect("note should be written");
        ...
        let new_text = "---\ntitle: Changed Title\n---\n\nBody.";
        std_fs::write(root.join("note.md"), new_text).expect("note should be overwritten");
        ...
        let index: crate::models::note::NoteIndex =
            serde_json::from_str(&std_fs::read_to_string(data_dir.join("index.json")).unwrap())
                .unwrap();
        assert_eq!(index.notes.len(), 1);
    ...
    #[tokio::test]
    async fn incremental_scan_removes_deleted_files_from_index() {
        ...
        std_fs::write(root.join("keep.md"), "# Keep").expect("keep note should be written");
        std_fs::write(root.join("delete.md"), "# Delete").expect("delete note should be written");
        ...
        // Delete one file
        std_fs::remove_file(root.join("delete.md")).expect("delete note should be removed");
    ...
    #[tokio::test]
    async fn incremental_scan_full_rebuild_on_root_path_change() {
        ...
        std_fs::write(root1.join("note1.md"), "---\ntitle: Note1\n---\n\nBody.")
            .expect("note1 should be written");
        std_fs::write(root2.join("note2.md"), "---\ntitle: Note2\n---\n\nBody.")
